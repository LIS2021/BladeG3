    ...
    match c with
      ...
      | VarAssign(id, r) ->
          ...
          (match List.find_opt (fun (x, r') -> x = id && r' = r) lprot with
             | Some(_) -> make_protect id p r
             | None -> c)
      | PtrAssign(e1, e2, l) -> c
      | ArrAssign(a, e1, e2) -> c
      | Seq(c1, c2) -> make_seq (protect_cmd c1 lprot) (protect_cmd c2 lprot)
      | If(e, c1, c2) -> make_if e (protect_cmd c1 lprot) (protect_cmd c2 lprot)
      | While(e, c) -> make_while e (protect_cmd c lprot)
      | Protect(id, p, r) -> c;;
