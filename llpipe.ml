...
let spec_list = [("--blade", Arg.Set enable_blade, "Enable blade optimization");
                 ...
                 ("-o", Arg.Set output_file, "Save the processed ll code in a file")];;

let () =
  Arg.parse spec_list (fun s -> input_file := s) usage_msg;
  let in_file = open_in ("test/" ^ !input_file ^ ".txt") in
  try
    match Parser.parse_channel in_file with
      | Some(ast) ->
          ...
          (match Lleval.build_ir final_ast !fancy with
            | Ok ir -> 
                Printf.printf "%s\n" ir;
                if !output_file then 
                  (let out_file = open_out (!input_file ^ ".ll") in
