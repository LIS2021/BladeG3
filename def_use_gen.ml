...
module type DefUseGen = sig
  ...
  val populate_graph: t -> cmd -> int -> unit

  val populate_graph_exp: generated -> node_type -> int -> node

  val populate_graph_rhs: generated -> node_type -> int -> node
end
  ...
  let rec populate_graph_exp (gen: generated) (e: expr) (cost: int) : node =
    ...
    match e with
      ...
      | BinOp (e1, e2, op) ->
        ...
        gen.g <- G.set_edge gen.g (b1, n_node) (cost_f cost); (* should these be -1 ?*)
        gen.g <- G.set_edge gen.g (b2, n_node) (cost_f cost);
        n_node
      | InlineIf (e1, e2, e3) ->
        let n_node = new_node gen (NExpr e) in
        ...
        gen.g <- G.set_edge gen.g (n1, n_node) (cost_f cost); (* should these be -1 ?*)
        gen.g <- G.set_edge gen.g (n2, n_node) (cost_f cost);
        gen.g <- G.set_edge gen.g (n3, n_node) (cost_f cost);
        n_node
  ...
  let rec populate_graph (gen: generated) (c: cmd) (cost: int): unit = 
    ...
    match c with 
      ...
      | While (e, c) ->
        ...
        gen.g <- G.set_edge gen.g (cond_node, sink) -1;
      | Protect (id, p, rhs) ->
        populate_graph_rhs gen rhs cost
      end